...
int main(int argc, char *argv[])
{
	...
	try
	{
		...
		else
		{
			...
			if(!in_bdlsg.empty() && !in_gleaf.empty() && leaf_scale != 0.0f && input_k_d != -1.0f && !isp0.empty() && !out_realistic_leaf.empty() && !is_palm_leaves)
			{
				BDLSkeletonLoader loader;
				loader.load_file(QString(in_bdlsg.c_str()));
				BDLSkeletonNode *skeleton = loader.construct_bdl_skeleton_tree();
            ...
            //testing palm tree
            if(!in_bdlsg.empty() && !in_gleaf.empty() && leaf_scale != 0.0f && !out_realistic_leaf.empty() && is_palm_leaves)
            {
