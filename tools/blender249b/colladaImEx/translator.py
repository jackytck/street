...
class MaterialNode(object):
	...
	def GenerateShader(self, bMaterial, updateShader):
		...
		#shader.AddValue(collada.DaeFxSyntax.TRANSPARENCY, bMaterial.alpha)
		shader.AddValue(collada.DaeFxSyntax.TRANSPARENCY, 0)#both branch and leaf need zero
	...
	def SaveToDae(self, bMaterial):
		...
		if daeEffect is None:
			...
			for mTex in textures:
				...
				if not mTex is None and mTex.tex.image != None:
					texture = mTex.tex
					textureNode = TextureNode(self.document)
